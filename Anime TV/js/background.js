...
setInterval(function(){
  console.log("Servis çalışıyor: " + new Date());
  storeNewAnimes();
}, bgTimeInterval);
...
function storeNewAnimes(){
  ...
  storage.get('newAnimes', function (data) {
    ...
    if($.isEmptyObject(data) || chrome.runtime.lastError) {
      console.log("newAnimes verisi yok");
      getNewAnimes(data);
     }
    // Eğer veriler storage'da varsa güncellemek için kontrol et
    else {
      console.log('newAnimes verisi cachede');
      var nextRunTime = new Date(data.newAnimes.nextrun).getTime();
      ...
      if (currentTime >= nextRunTime){
        ...
        console.log('Henüz API güncellenmedi. nextrun: ' + new Date(nextRunTime));
      }
    }
  });
}
...
function getNewAnimes(oldAnimes){
  $.ajax({
    ...
    success: function (response) {
      ...
      storage.set({'newAnimes': newAnimes});
